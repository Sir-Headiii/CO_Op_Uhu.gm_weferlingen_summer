params ["_searchlight", ["_code", {(damage _this < 0.1) && !((behaviour _this) isEqualTo "COMBAT")}], ["_turnDegrees", 0.5], ["_turnStart", 275], ["_turnEnd", 445], ["_sleepTime", 0.1], ["_allTheWay", false]];
...
if (_allTheWay) then {
	...
	while {(_incrementer >= 0) && _condition} do {
		_searchlight doWatch (_searchlight getRelPos [1000, _incrementer]);
